...
$resultCron = $db->db()->from('doc_res')
	...
	->andWhere('client_validade_certificado')->greaterThan(date('Y-m-d'))
    ->limit(50)
    ->select()
    ->all();
...
if($resultCron){
    foreach($resultCron as $rCron){
        ...
        $resultCronDoc = $db->db()->from('doc')
            ->where('doc_id_client')->is($rCron->doc_res_id_client)
            ->andWhere('doc_chave')->is($rCron->doc_res_chave)
            ->limit(1)
            ->select()
            ->all();
        if($resultCronDoc){
