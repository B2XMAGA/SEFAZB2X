...
echo "<meta HTTP-EQUIV='refresh' CONTENT='2;URL=dfeS3ConverterEnvio.php'>";
...
if ($resultCron) {
    foreach ($resultCron as $rCron) {
		$result = $db->db()->from('doc')
			->where('doc_id')->is($rCron->doc_id)
			->limit(1)
			->select()
			->all();
		if($result){
			foreach($result as $r){
				...
				if ($ks3->uploadXMLBase64($r->doc_file, $r->doc_id_client . '/'.$r->doc_ide.'.xml')) {
					$form_doc['doc_status_s3'] = 1;
				
					$db->db()->update('doc')->where('doc_id')->is($r->doc_id)->set($form_doc);
				} else {
					echo 'ERRO';
				}
			}
