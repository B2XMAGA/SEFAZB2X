...
//echo "<meta HTTP-EQUIV='refresh' CONTENT='10;URL=dfeS3Extract.php'>";
...
$resultCron = $db->db()->from('dfe')
    ...
    ->limit(50)
    ->select(['dfe_id', 'dfe_ide_client', 'dfe_nsu', 'dfe_status', 'dfe_schema', 'dfe_url_s3', 'dfe_md5'])
    ->all();
...
if ($resultCron) {
    foreach ($resultCron as $rCron) {
		...
		$urlPadraao = 'https://s3xmls.ams3.cdn.digitaloceanspaces.com/'.$rCron->dfe_ide_client.'/'.$rCron->dfe_md5.'.xml';
		
		$XMLN = base64_decode($rCron->dfe_doc);
		$XML = simplexml_load_string($XMLN);
		...
		if ($rCron->dfe_schema == 'resNFe_v1.01.xsd') {
            ...
            if (!$resultNFeResumes) {
                ...
                $form_nfe_resume['doc_res_ide'] = $rCron->dfe_md5;
				$form_nfe_resume['doc_res_status_s3'] = '1';
        ...
        if ($rCron->dfe_schema == 'procNFe_v4.00.xsd') {
			...
			$form_nfe['doc_ide'] = $rCron->dfe_md5;
			$form_nfe['doc_status_s3'] = '1';
        ...
        if ($rCron->dfe_schema == 'resEvento_v1.01.xsd') {
            ...
            $form_evento['eventos_ide'] = $rCron->dfe_md5;
			$form_evento['eventos_status_s3'] = '1';
